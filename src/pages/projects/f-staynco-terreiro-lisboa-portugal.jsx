...
const TerreiroProject = () => {
  const allImagesQuery = graphql`
    query {
      allFile(
        filter: {
          extension: { regex: "/(jpg)|(png)|(jpeg)/" }
          relativeDirectory: { eq: "projects/terreiro-do-paco-lisbon" }
        }
        sort: { fields: name }
      ) {
        edges {
  ...
  const projectInfo = projectsData.filter(
    project => project.name === "f-staynco-terreiro-lisboa-portugal"
  )
  return (
    <Layout>
...
export default TerreiroProject
